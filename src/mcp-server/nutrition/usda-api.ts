...
export class USDAClient {
     ...
     * 搜索食物并返回最佳匹配
     */
    async searchFood(query: string): Promise<NutritionData | null> {
        try {
        ...
        console.error('🔍 [USDA] Food options:', 
        filteredFoods.slice(0, 5).map((f: USDAFood) => f.description)
        );
    ...
    // 新增：智能食物选择方法
    private selectBestFood(foods: USDAFood[], query: string): USDAFood {
     ...
     * 通过 FDC ID 获取详细信息
     */
    async getFoodById(fdcId: number): Promise<NutritionData | null> {
        try {
     ...
     * 从 USDA 数据提取营养成分
     */
    private extractNutrition(food: USDAFood): NutritionData {
 ...
 * 将中文食材名转换为英文
 */
export function translateFoodName(chineseName: string): string {
